...
The [simple features standard](http://www.opengeospatial.org/standards/sfa)  has the following limitations meaning that it cannot represent in-full every-day objects from GPS, `rgl`, `ggplot2`/`ggvis`, `spatstat`, `maps`, TopoJSON, CAD drawings, or from 3D or general model structures. 
...
Because of these limitations the translation between geo-spatial forms and the graphics and data grammars is disjointed and sometimes awkward, relying on localized implementations that can be lossy or inefficient, require 3rd party workflows, and sometimes involve unnecessary tasks. 

Many translation patterns that use simple features at some point result in  loss of information that does not fit within the model, requiring complicated workarounds to preserve it. These translations, ... common ... examples .. in-memory structural representations, and serialized forms within file formats, coordinate system geometry transformations, and topological or shape-modifying transformations. 
...
The `PATH` models consists of three tables to store the objects ("features"), paths, and vertices, and a fourth table `path_link_vertex` allows de-duplication of the vertices (here in X-Y geometry). 
