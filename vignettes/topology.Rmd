...
The key aspect here is that each component entity is named formally named, and has a unique ID that we persist for subsequent usage. This persistence is required as it records relationships between existing entitites *implicitly* rather than baking data into structure that are both *explicit*  and also discard a lot of information about relationships. 
...
Currently we haven't yet normalized the arcs  ... and there's a bug somewhere screwing this up 

```{r}
arcs <- sc_arc_base(x$path_link_vertex, nodes)
distinct(arcs, arc)
