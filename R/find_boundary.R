sc_identify_object_boundary <- function(x, ...) {
  ...
  boundary_edges <- purrr::map(split(object_link_edge, object_link_edge$object_),
                               ~.x %>% dplyr::group_by(.data$edge_) %>%
    dplyr::filter(n() < 2) %>% dplyr::ungroup() %>% dplyr::distinct(.data$edge_)) %>%
    dplyr::bind_rows()


  boundary_edges %>%
    dplyr::inner_join(segment[c(".vertex0", ".vertex1", "edge_")]) %>%
    tidyr::gather(.data$node, .data$vertex_, -.data$edge_) %>% dplyr::inner_join(x$vertex)
